    ...
    return (
        <div className="space-y-4">
                ...
                <div className="surface rounded-2xl overflow-hidden border border-white/5">
                    <table className="w-full text-sm">
                        <thead className="bg-white/5">
                            <tr>
                                <th className="p-3 text-left text-subtle uppercase text-xs font-bold">Abschlag</th>
                                <th className="p-3 text-right text-subtle uppercase text-xs font-bold">Soll</th>
                                <th className="p-3 text-right text-subtle uppercase text-xs font-bold">Gezahlt</th>
                                <th className="p-3 text-right text-subtle uppercase text-xs font-bold">Zahldatum</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {entries.map(e => (
                                <tr key={String(e.month)}>
                                    ...
                                    <td className="p-3 text-right font-mono text-muted">{e.expectedAmount.toFixed(2)} €</td>
            ...
            {!balance ? (
                ...
                <div className="border-t border-white/10 pt-4 space-y-2">
                    <div className="flex justify-between text-sm">
                        <span className="text-muted">Rechnungsbetrag</span>
                        <span className="font-mono">{bill.totalAmount.toFixed(2)} €</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-muted">abzgl. {describeAdvance(bill)}</span>
            ...
            {bill.newAdvanceAmount != null && bill.newAdvanceFrom && (
                <p className="text-sm text-muted bg-white/5 rounded-2xl p-3">
