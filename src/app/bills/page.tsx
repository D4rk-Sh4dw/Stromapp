    ...
    return (
        <div className="space-y-8">
            <header className="flex justify-between items-end">
                <div>
                    <h1 className="text-2xl font-semibold tracking-tight">Meine Rechnungen</h1>
                    <p className="text-subtle mt-1">Übersicht und Download vergangener Abrechnungen.</p>
                </div>
            </header>

            <div className="grid grid-cols-1 gap-4">
                {loading ? (
                    <div className="p-20 text-center text-subtle italic">Lade Abrechnungen...</div>
                ) : bills.length === 0 ? (
                    <div className="surface p-20 text-center rounded-2xl border-dashed border-2 border-white/5">
                        <FileText className="w-12 h-12 text-white/10 mx-auto mb-4" />
                        <p className="text-subtle">Noch keine Abrechnungen vorhanden.</p>
                    </div>
                ) : (
                    bills.map((bill, i) => (
                        <motion.div
                            ...
                            className="surface p-6 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between group hover:border-primary/30 transition-all gap-4 sm:gap-0"
                        >
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h3 className="font-semibold text-lg tracking-tight">Abrechnung {new Date(bill.startDate).toLocaleDateString()} - {new Date(bill.endDate).toLocaleDateString()}</h3>
                            ...
                            <div className="w-full sm:w-auto grid grid-cols-2 sm:flex items-center gap-y-6 gap-x-4 sm:gap-12 pt-4 sm:pt-0 border-t sm:border-0 border-white/5 sm:border-transparent">
                                <div className="text-left sm:text-right">
                                    <p className="text-xs text-subtle">Verbrauch</p>
                                    <p className="font-bold text-white/80">{bill.totalUsage.toFixed(1)} kWh</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-subtle">Gesamtbetrag</p>
                                    <p className="text-xl font-semibold text-primary">{bill.totalAmount.toFixed(2)} €</p>
                                    {getBillBalance(bill) && (
                                ...
                                <div className="flex gap-2 justify-end">
                                    <button
                                        ...
                                        className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-colors border border-white/5 text-primary"
            ...
            {user?.autoBilling && (
                <div className="surface p-8 rounded-2xl border-dashed border-2 border-white/5 flex flex-col items-center justify-center py-12">
                    <Clock className="w-12 h-12 text-white/10 mb-4" />
                    <h3 className="text-lg font-bold">Auto-Abrechnung ist aktiv</h3>
                    <p className="text-subtle text-sm mt-1 text-center max-w-lg italic">
